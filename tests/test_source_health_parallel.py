...
def test_career_site_health_checks_run_concurrently(monkeypatch,tmp_path):
    ...
    calls=[]
    def synchronized(company,url,pattern,timeout):
        calls.append(company)
        barrier.wait()
        return {"status":"ok","http_status":200,"matching_job_links":1}
    monkeypatch.setattr(source_health,"validate_career_site",synchronized)
    report=source_health.run("sources.json",timeout=1)
    rows=[r for r in report["sources"] if r["provider"]=="career_site"]
    assert len(rows)==2
