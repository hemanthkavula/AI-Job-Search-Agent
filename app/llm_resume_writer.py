...
Your FIRST draft must be submission-ready. Perform the JD analysis and quality review internally before returning the resume. Do not rely on a later regeneration pass to repair avoidable omissions.
...
If audit feedback is supplied, correct every failed gate specifically while preserving strong truthful content from the previous attempt. Never ignore the audit feedback and never invent unsupported evidence to satisfy it.
Before returning JSON, silently self-check: exact 8/7/6 bullet counts; important JD keywords covered where supported; skills have experience evidence; no unsupported technologies or claims; metric limits satisfied; no repeated boilerplate; summary aligned to target role; employer domains preserved; dates/titles/education unchanged. Correct any issue before returning the draft.
