...
def fetch_jobs(company:str,search_url:str,timeout:int=25)->list[dict]:
    ...
    for page,j in candidates:
        ...
        out.append({"external_id":f"cornerstone:{tenant}:{ident}","source":"cornerstone","company_key":company,"title":title,"location":None,"url":url,"original_url":url,"ats_provider":"cornerstone","ats_identifier":tenant,"job_id":ident,"description":desc,"description_complete":bool(desc),"updated_at":j.get("datePosted") or j.get("validThrough")})
    return out
