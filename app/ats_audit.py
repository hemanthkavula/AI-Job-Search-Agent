...
def ats_audit(job,profile,resume_path):
 ...
 paras=Document(resume_path).paragraphs;by_company=_experience_bullets(paras);bullets=[b for xs in by_company.values() for b in xs];counts={k:len(v) for k,v in by_company.items()};bullet_count_score=100 if counts==EXPECTED_COUNTS else 60;metric_counts={c:sum(_is_metric_bullet(b) for b in xs) for c,xs in by_company.items()};metric_lines=sum(metric_counts.values());metric_violations={c:{"count":metric_counts[c],"limit":limit} for c,limit in METRIC_LIMITS.items() if metric_counts[c]>limit};unapproved_metrics=_unapproved_metric_claims(by_company);accomplishment_score=min(100,60+min(metric_lines,4)*10)
 if metric_violations:accomplishment_score=max(40,accomplishment_score-20*len(metric_violations))
 repeated_phrases,repeated_openings=_repetition_findings(bullets);repetition_score=100-min(35,len(repeated_phrases)*7)-min(20,sum(v-4 for v in repeated_openings.values())*4);repetition_score=max(40,repetition_score);skills_text=_technical_skills_text(paras);listed,evidenced,evidence_gaps,evidence_score=_evidence_coverage(jd_terms,skills_text,bullets);readability_score=_readability_score(bullets,repetition_score);human_quality_score=round(readability_score*.40+repetition_score*.25+accomplishment_score*.20+bullet_count_score*.15);compatibility_score=round(keyword_coverage*.60+title_alignment*.15+section_score*.10+bullet_count_score*.10+5);score=round(compatibility_score*.70+human_quality_score*.30)
 discovery_score=getattr(job,"discovery_score",None);gates={"discovery":discovery_score is None or discovery_score>=80,"structure":counts==EXPECTED_COUNTS,"metrics":not metric_violations and not unapproved_metrics,"repetition":repetition_score>=85,"technology_evidence":evidence_score>=EVIDENCE_TARGET and not evidence_gaps,"human_quality":human_quality_score>=HUMAN_QUALITY_TARGET};quality_gate=all(gates.values());passed=score>=ATS_TARGET and keyword_coverage>=95 and not missing and quality_gate
 return {"passed":passed,"internal_ats_score":score,"target":ATS_TARGET,"compatibility_score":compatibility_score,"human_quality_score":human_quality_score,"human_quality_target":HUMAN_QUALITY_TARGET,"readability_score":readability_score,"keyword_coverage":round(keyword_coverage),"title_alignment":round(title_alignment),"section_score":round(section_score),"accomplishment_score":round(accomplishment_score),"targeted_jd_terms":targeted,"ignored_alternative_terms":[t for t in raw_targeted if t not in targeted],"missing_jd_keywords":missing,"technical_skills_jd_terms":listed,"experience_evidenced_jd_terms":evidenced,"skills_without_experience_evidence":evidence_gaps,"technology_evidence_coverage":evidence_score,"technology_evidence_target":EVIDENCE_TARGET,"metric_bearing_bullets":metric_lines,"metric_counts_by_employer":metric_counts,"metric_violations":metric_violations,"unapproved_metric_claims":unapproved_metrics,"approved_metric_patterns":APPROVED_METRIC_PATTERNS,"bullet_counts":counts,"bullet_count_score":bullet_count_score,"skills_taxonomy_score":100,"repetition_score":repetition_score,"repeated_phrases":repeated_phrases,"repeated_opening_verbs":repeated_openings,"discovery_score":discovery_score,"quality_gate_passed":quality_gate,"quality_gates":gates,"status":"ATS_PASS" if passed else "HOLD_ATS_REVIEW","note":"Internal ATS/quality estimate only. Aliases and equivalent DE concepts are normalized; alternative programming languages are not all required. New numerical claims must match approved evidence."}
