...
def ats_audit(job,profile,resume_path):
 ...
 discovery_score=getattr(job,"discovery_score",None);gates={"discovery":discovery_score is None or discovery_score>=80,"structure":counts==EXPECTED_COUNTS,"metrics":not metric_violations and not unapproved_metrics,"repetition":repetition_score>=85,"human_quality":human_quality_score>=HUMAN_QUALITY_TARGET};quality_gate=all(gates.values());passed=score>=ATS_TARGET and keyword_coverage>=95 and not missing and quality_gate
 return {"passed":passed,"internal_ats_score":score,"target":ATS_TARGET,"compatibility_score":compatibility_score,"human_quality_score":human_quality_score,"human_quality_target":HUMAN_QUALITY_TARGET,"readability_score":readability_score,"keyword_coverage":round(keyword_coverage),"title_alignment":round(title_alignment),"section_score":round(section_score),"accomplishment_score":round(accomplishment_score),"targeted_jd_terms":targeted,"ignored_alternative_terms":[t for t in raw_targeted if t not in targeted],"missing_jd_keywords":missing,"metric_bearing_bullets":metric_lines,"metric_counts_by_employer":metric_counts,"metric_violations":metric_violations,"unapproved_metric_claims":unapproved_metrics,"approved_metric_patterns":APPROVED_METRIC_PATTERNS,"bullet_counts":counts,"bullet_count_score":bullet_count_score,"skills_taxonomy_score":100,"repetition_score":repetition_score,"repeated_phrases":repeated_phrases,"repeated_opening_verbs":repeated_openings,"discovery_score":discovery_score,"quality_gate_passed":quality_gate,"quality_gates":gates,"status":"ATS_PASS" if passed else "HOLD_ATS_REVIEW","note":"Internal ATS/JD-quality estimate only. Complete-JD keyword alignment, structure, metrics, repetition and human readability determine release."}
