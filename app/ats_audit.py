...
# Only concrete metrics already approved as candidate evidence may appear in generated bullets.
APPROVED_METRIC_PATTERNS={
    "Fidelity Investments":[r"2\.5\s*[–-]\s*3\s*(?:m|million)",r"400\s*[–-]\s*500\s*gb",r"30\s*%"],
    "Cigna Healthcare":[r"2\s*[–-]\s*3\s*(?:m|million)",r"25\s*[–-]\s*30\s*%"],
    "Target Corporation":[]
}
...
def _contains(text,term):return bool(re.search(r"(?<![a-z0-9])"+re.escape(_norm(term))+r"(?![a-z0-9])",_norm(text)))
...
def _unapproved_metric_claims(by_company):
    ...
    for company,bullets in by_company.items():
        for bullet in bullets:
            s=_metric_sanitized(bullet)
            metric_tokens=[m.group(0) for pat in METRIC_TOKEN_PATTERNS for m in re.finditer(pat,s,re.I)]
            if metric_tokens and not _has_approved_metric(company,bullet):findings.append({"company":company,"metrics":list(dict.fromkeys(metric_tokens)),"bullet":bullet})
            elif metric_tokens:
                # An approved metric does not authorize an additional invented metric in the same bullet.
                approved_removed=s
                for pat in APPROVED_METRIC_PATTERNS.get(company,[]):approved_removed=re.sub(pat," ",approved_removed,flags=re.I)
                extras=[m.group(0) for pat in METRIC_TOKEN_PATTERNS for m in re.finditer(pat,approved_removed,re.I)]
...
def _evidence_coverage(jd_terms,skills_text,bullets):
    experience_text="\n".join(bullets);listed=[t for t in jd_terms if _contains(skills_text,t)];evidenced=[t for t in listed if _contains(experience_text,t)];gaps=[t for t in listed if t not in evidenced]
...
def _readability_score(bullets,repetition_score):
    ...
    lengths=[len(re.findall(r"\b\w+[+#.-]*\b",b)) for b in bullets];long_count=sum(n>38 for n in lengths);very_long=sum(n>50 for n in lengths);avg=sum(lengths)/len(lengths);score=100-min(25,long_count*3)-min(20,very_long*5)
    if avg>34:score-=10
    return max(40,round(min(score,repetition_score)))

def ats_audit(job,profile,resume_path):
    text=document_text(resume_path);low=_norm(text);verified=jd_keywords(job.description,profile);inferred=inferable_terms(job.description);jd_terms=jd_skill_terms(job.description);targeted=list(dict.fromkeys(verified+inferred+jd_terms));present=[k for k in targeted if _contains(low,k)];missing=[k for k in targeted if not _contains(low,k)];keyword_coverage=100*len(present)/max(1,len(targeted))
    ...
    paras=Document(resume_path).paragraphs;by_company=_experience_bullets(paras);bullets=[b for xs in by_company.values() for b in xs];counts={k:len(v) for k,v in by_company.items()};bullet_count_score=100 if counts==EXPECTED_COUNTS else 60
    metric_counts={c:sum(_is_metric_bullet(b) for b in xs) for c,xs in by_company.items()};metric_lines=sum(metric_counts.values());metric_violations={c:{"count":metric_counts[c],"limit":limit} for c,limit in METRIC_LIMITS.items() if metric_counts[c]>limit};unapproved_metrics=_unapproved_metric_claims(by_company)
    ...
    repetition_score=max(40,repetition_score);skills_text=_technical_skills_text(paras);listed,evidenced,evidence_gaps,evidence_score=_evidence_coverage(jd_terms,skills_text,bullets);readability_score=_readability_score(bullets,repetition_score);taxonomy_score=100;human_quality_score=round(readability_score*.40+repetition_score*.25+accomplishment_score*.20+bullet_count_score*.15);compatibility_score=round(keyword_coverage*.60+title_alignment*.15+section_score*.10+bullet_count_score*.10+taxonomy_score*.05);score=round(compatibility_score*.70+human_quality_score*.30)
    discovery_score=getattr(job,"discovery_score",None);discovery_gate=(discovery_score is None or discovery_score>=80);structural_gate=counts==EXPECTED_COUNTS;metric_gate=not metric_violations and not unapproved_metrics;repetition_gate=repetition_score>=85;evidence_gate=evidence_score>=EVIDENCE_TARGET and not evidence_gaps;human_gate=human_quality_score>=HUMAN_QUALITY_TARGET;quality_gate=discovery_gate and structural_gate and metric_gate and repetition_gate and evidence_gate and human_gate;passed=score>=ATS_TARGET and keyword_coverage>=95 and not missing and quality_gate
    return {"passed":passed,"internal_ats_score":score,"target":ATS_TARGET,"compatibility_score":compatibility_score,"human_quality_score":human_quality_score,"human_quality_target":HUMAN_QUALITY_TARGET,"readability_score":readability_score,"keyword_coverage":round(keyword_coverage),"title_alignment":round(title_alignment),"section_score":round(section_score),"accomplishment_score":round(accomplishment_score),"targeted_jd_terms":targeted,"missing_jd_keywords":missing,"technical_skills_jd_terms":listed,"experience_evidenced_jd_terms":evidenced,"skills_without_experience_evidence":evidence_gaps,"technology_evidence_coverage":evidence_score,"technology_evidence_target":EVIDENCE_TARGET,"metric_bearing_bullets":metric_lines,"metric_counts_by_employer":metric_counts,"metric_violations":metric_violations,"unapproved_metric_claims":unapproved_metrics,"approved_metric_patterns":APPROVED_METRIC_PATTERNS,"bullet_counts":counts,"bullet_count_score":bullet_count_score,"skills_taxonomy_score":taxonomy_score,"repetition_score":repetition_score,"repeated_phrases":repeated_phrases,"repeated_opening_verbs":repeated_openings,"discovery_score":discovery_score,"quality_gate_passed":quality_gate,"quality_gates":{"discovery":discovery_gate,"structure":structural_gate,"metrics":metric_gate,"repetition":repetition_gate,"technology_evidence":evidence_gate,"human_quality":human_gate},"status":"ATS_PASS" if passed else "HOLD_ATS_REVIEW","note":"Internal ATS/quality estimate only. Generated numerical claims must match approved candidate evidence; new metrics trigger regeneration."}
