...
def ats_audit(job,profile,resume_path):
 ...
 paras=Document(resume_path).paragraphs;by_company=_experience_bullets(paras);bullets=[b for xs in by_company.values() for b in xs];experience_text="\n".join(bullets);plan=build_coverage_plan(job,profile);must_cover_terms=plan.get("must_cover_terms",[]);experience_covered=[t for t in must_cover_terms if _contains(experience_text,t)];experience_gaps=[t for t in must_cover_terms if not _contains(experience_text,t)];experience_coverage=100 if not must_cover_terms else 100*len(experience_covered)/len(must_cover_terms);counts={k:len(v) for k,v in by_company.items()};bullet_count_score=100 if counts==EXPECTED_COUNTS else 60;metric_counts={c:sum(_is_metric_bullet(b) for b in xs) for c,xs in by_company.items()};metric_lines=sum(metric_counts.values());metric_violations={c:{"count":metric_counts[c],"limit":limit} for c,limit in METRIC_LIMITS.items() if metric_counts[c]>limit};unapproved_metrics=_unapproved_metric_claims(by_company);accomplishment_score=min(100,60+min(metric_lines,4)*10)
