...
def prepare(report_path,output_path="generated/application_manifest.json",min_score=80,debug_company=None,single_attempt=False):
    ...
    for item in report.get("results",[]):
        ...
        if score < min_score:
            print(f"SKIP {company} - score {score} < {min_score}",flush=True);continue
        ...
        try:
            print("Calling OpenAI resume writer...",flush=True)
            ...
            print(f"DOCX generated: {resume}",flush=True)
            audit=ats_audit(job,profile,resume);attempts=1
            max_attempts=1 if single_attempt else 5
            while not audit["passed"] and attempts < max_attempts:
                print(f"ATS score {audit.get('internal_ats_score')} did not pass; regeneration {attempts+1}/{max_attempts}...",flush=True)
            ...
            print(f"DONE {job.company} | passed={audit['passed']} | attempts={attempts}",flush=True)
...
if __name__=="__main__":
    ap=argparse.ArgumentParser();ap.add_argument("--report",default="generated/daily_jobs.json");ap.add_argument("--output",default="generated/application_manifest.json");ap.add_argument("--min-score",type=int,default=80);ap.add_argument("--debug-company");ap.add_argument("--single-attempt",action="store_true");a=ap.parse_args()
    rows=prepare(a.report,a.output,a.min_score,a.debug_company,a.single_attempt)
