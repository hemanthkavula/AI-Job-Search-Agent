...
function render(){let q=search.value.toLowerCase(),f=filter.value,s=sourceFilter.value;x=rows.filter(r=>(!q||(r.company+" "+r.title+" "+r.portal).toLowerCase().includes(q))&&(!f||r.stage===f)&&(!s||r.portal===s));jobs.innerHTML=x.map(r=>'<div class="row"><div class="companyWrap"><div class="avatar">'+esc(initial(r.company))+'</div><div><div class="company">'+esc(r.company)+'</div><div class="role">'+esc(r.title)+'</div></div></div><div><span class="badge '+cls(r.stage)+'">'+esc(r.stage)+'</span></div><div class="portal">'+esc(r.portal)+'</div><div class="date">'+(r.stage==="Applied"?dt(r.applied_at):"—")+'</div><div class="actions">'+(r.url?'<a class="btn primary" href="'+esc(r.url)+'" target="_blank">↗ Open Job</a>':'')+(r.stage!=="Applied"?'<button class="btn appliedBtn" data-job-key="'+esc(r.key)+'">✓ Mark Applied</button>':'<span class="badge applied">✓ Applied</span>')+(r.resume_url?'<a class="btn" href="'+r.resume_url+'" target="_blank">▤ View Resume</a>':'<span class="btn" style="opacity:.5">Resume unavailable</span>')+'<div class="menuWrap"><button class="btn moreBtn" data-menu="1">⋮</button><div class="menu"><button class="btn deleteBtn" data-delete-key="'+esc(r.key)+'">Delete application</button></div></div></div></div>').join("")||'<div class="empty">No applications in this view.</div>'}
