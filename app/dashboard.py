...
async function markApplied(key){if(!confirm("Mark this application as Applied?"))return;let r=await fetch("/api/applications/"+encodeURIComponent(key)+"/confirm-submitted",{method:"POST"});if(!r.ok){alert("Could not update status.");return}await load()}
function render(){let q=search.value.toLowerCase(),f=filter.value,x=rows.filter(r=>(!q||(r.company+" "+r.title).toLowerCase().includes(q))&&(!f||r.stage===f));jobs.innerHTML=x.map(r=>'<div class="row"><div><div class="company">'+esc(r.company)+'</div><div class="role">'+esc(r.title)+'</div><div class="portal">'+esc(r.portal)+'</div></div><div><span class="badge '+cls(r.stage)+'">'+esc(r.stage)+'</span></div><div class="resume">'+(r.resume_url?'<a class="btn" href="'+r.resume_url+'" target="_blank">View resume</a>':'PDF unavailable')+'</div><div class="date">'+(r.stage==="Applied"?dt(r.applied_at):"—")+'</div><div class="actions">'+(r.url?'<a class="btn primary" href="'+esc(r.url)+'" target="_blank">Open job / Apply</a>':'')+(r.stage!=="Applied"?'<button class="btn appliedBtn" onclick='+JSON.stringify("markApplied("+JSON.stringify(r.key)+")")+'>Mark Applied</button>':'')+'</div></div>').join("")||'<div class="empty">No applications in this view.</div>'}
async function load(){let d=await fetch("/api/applications",{cache:"no-store"}).then(r=>r.json());rows=d.applications;Object.entries(d.counts).forEach(([k,v])=>document.getElementById(k).textContent=v);render()}search.oninput=render;filter.onchange=render;load();setInterval(load,10000);
</script></body></html>""")
