...
*{box-sizing:border-box}body{margin:0;font-family:Inter,Segoe UI,Arial,sans-serif;background:#07111f;color:#e8eef8}.app{display:grid;grid-template-columns:220px 1fr;min-height:100vh}.side{background:#0c1828;border-right:1px solid #1c2a3c;padding:22px 14px;position:sticky;top:0;height:100vh}.brand{font-size:21px;font-weight:850;padding:0 8px 26px}.brand small{display:block;color:#8fa0b8;font-size:11px;font-weight:500;margin-top:4px}.nav{display:grid;gap:7px}.nav div{padding:12px 13px;border-radius:8px;color:#c7d2e2;font-size:13px}.nav .active{background:#173967;color:#fff}.sched{position:absolute;bottom:22px;left:14px;right:14px;background:#101f31;border:1px solid #1d3046;border-radius:9px;padding:12px;font-size:11px;color:#9fb0c7}.sched b{display:block;color:#4ee4a1;margin-bottom:7px}.main{padding:24px;max-width:1700px;width:100%}.top{display:flex;justify-content:space-between;align-items:center;margin-bottom:24px}.top h1{font-size:24px;margin:0 0 5px}.muted{color:#8fa0b8;font-size:12px}.datepick{background:#0d1a2b;border:1px solid #263950;color:#e7eef8;border-radius:8px;padding:11px 14px}.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:20px}.stat{background:#0d1a2b;border:1px solid #263950;border-radius:10px;padding:18px}.stat span{color:#a9b8ca;font-size:12px}.stat b{display:block;font-size:26px;margin-top:6px}.tabsbar{display:flex;justify-content:space-between;gap:14px;align-items:center;margin-bottom:18px}.tabs{display:flex;gap:8px;overflow:auto}.tab{border:1px solid #263950;background:#0d1a2b;color:#aebdd0;border-radius:8px;padding:10px 15px;white-space:nowrap;cursor:pointer}.tab.active{background:#2684ff;color:#fff;border-color:#2684ff}.filters{display:flex;gap:8px}.filters input,.filters select{background:#0d1a2b;border:1px solid #263950;color:#dce6f4;border-radius:8px;padding:10px 12px}.section{background:#0b1726;border:1px solid #21344a;border-radius:10px;margin-bottom:18px;overflow:hidden}.sectionHead{padding:15px 17px;border-bottom:1px solid #1d3044;font-weight:800;display:flex;justify-content:space-between}.pipes{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;padding:14px}.pipe{border:1px solid #263950;background:#0e1c2e;border-radius:9px;padding:14px}.pipe b{font-size:13px}.pipe .green{color:#42dda0}.pipe div{margin-top:7px;color:#9fb0c7;font-size:11px}.row{display:grid;grid-template-columns:1.2fr 1fr .9fr .8fr .9fr .8fr 1fr 1.25fr;gap:12px;align-items:center;padding:12px 17px;border-bottom:1px solid #17283a;font-size:12px}.head{background:#101f31;color:#9fb0c7;font-size:10px;text-transform:uppercase}.title{font-weight:750;color:#f3f7fc}.company{color:#b5c3d4}.badge{display:inline-block;padding:5px 9px;border-radius:999px;font-size:10px;font-weight:800;background:#17304d}.ready-to-apply,.applying{background:#0d4637;color:#62e5b0}.applied{background:#173b69;color:#74b4ff}.needs-attention,.verify-submission{background:#522d31;color:#ff8f91}.retrying{background:#392d61;color:#c6a8ff}.actions{display:flex;gap:6px;flex-wrap:wrap}.btn{border:1px solid #30465f;background:#13253a;color:#dce7f5;text-decoration:none;padding:7px 9px;border-radius:7px;font-size:10px;cursor:pointer}.btn.primary{background:#1677e8;border-color:#2684ff;color:#fff}.empty{padding:28px;text-align:center;color:#8192a8}.menuWrap{position:relative}.menu{display:none;position:absolute;right:0;top:34px;background:#102033;border:1px solid #2a4058;padding:6px;border-radius:8px;z-index:5}.menu.open{display:block}.deleteBtn{color:#ff8f91}.count{color:#8fa0b8;font-size:11px}
@media(max-width:1050px){.app{grid-template-columns:1fr}.side{display:none}.stats{grid-template-columns:1fr 1fr}.pipes{grid-template-columns:1fr}.row{grid-template-columns:1fr 1fr}.head{display:none}.tabsbar{align-items:stretch;flex-direction:column}}@media(max-width:650px){.main{padding:15px}.stats{grid-template-columns:1fr 1fr}.filters{flex-direction:column}.row{grid-template-columns:1fr}}
</style></head><body><div class="app"><aside class="side"><div class="brand">💼 Auto Apply<small>Job Application Manager</small></div><div class="nav"><div class="active">⌂ &nbsp; Dashboard</div><div>▣ &nbsp; Jobs</div><div>⌘ &nbsp; Pipelines</div><div>□ &nbsp; Scheduler</div><div>⚙ &nbsp; Settings</div></div><div class="sched"><b>● Scheduler Active</b>Cloudflare schedule enabled</div></aside><main class="main"><div class="top"><div><h1>👋 Good Morning!</h1><div class="muted">Here's your job application overview.</div></div><div class="datepick" id="todayLabel"></div></div>
...
<div class="tabsbar"><div class="tabs" id="tabs"></div><div class="filters"><input id="search" placeholder="⌕ Search jobs, companies..."><select id="filter"><option value="">All statuses</option><option>Ready to apply</option><option>Applied</option><option>Applying</option><option>Retrying</option><option>Needs attention</option><option>Verify submission</option></select></div></div>
<section class="section"><div class="sectionHead"><span id="pipeTitle">Pipeline Runs</span><span class="count" id="pipeCount"></span></div><div class="pipes" id="pipes"></div></section>
<section class="section"><div class="sectionHead"><span id="jobsTitle">Jobs</span><span class="count" id="jobCount"></span></div><div class="row head"><div>Job Title</div><div>Company</div><div>Location</div><div>Source</div><div>Pipeline</div><div>Status</div><div>Date Created</div><div>Actions</div></div><div id="jobs"></div></section></main></div><script>
let rows=[],pipelineRows=[],selectedDate="";const esc=s=>String(s??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));const cls=s=>String(s).toLowerCase().replaceAll(" ","-");const dateKey=v=>{if(!v)return"";let d=new Date(v);return isNaN(d)?String(v).slice(0,10):d.toLocaleDateString("en-CA")};const niceDate=v=>{if(!v)return"—";let d=new Date(v);return isNaN(d)?v:d.toLocaleString([],{month:"short",day:"numeric",year:"numeric",hour:"numeric",minute:"2-digit"})};const labelDate=k=>{let d=new Date(k+"T12:00:00");return d.toLocaleDateString([],{month:"short",day:"numeric"})};const pipelineName=id=>id?("Run "+id.slice(9,13)):"—";
...
function renderTabs(){let dates=[...new Set(rows.map(r=>dateKey(r.created)).filter(Boolean))].sort().reverse();if(!selectedDate&&dates.length)selectedDate=dates[0];tabs.innerHTML=dates.slice(0,6).map((d,i)=>'<button class="tab '+(d===selectedDate?"active":"")+'" data-date="'+d+'">'+(i===0?"Today":labelDate(d))+'</button>').join("")+'<button class="tab '+(!selectedDate?"active":"")+'" data-date="">All Dates</button>'}
function render(){let q=search.value.toLowerCase(),f=filter.value;let x=rows.filter(r=>(!selectedDate||dateKey(r.created)===selectedDate)&&(!q||(r.company+" "+r.title+" "+r.portal).toLowerCase().includes(q))&&(!f||r.stage===f));counts(x);jobCount.textContent="Showing "+x.length+" jobs";jobsTitle.textContent=selectedDate?"Jobs — "+labelDate(selectedDate):"Jobs — All Dates";jobs.innerHTML=x.map(r=>'<div class="row"><div class="title">'+esc(r.title)+'</div><div class="company">'+esc(r.company)+'</div><div>'+esc(r.location||"—")+'</div><div>'+esc(r.portal)+'</div><div>'+esc(pipelineName(r.pipeline))+'</div><div><span class="badge '+cls(r.stage)+'">'+esc(r.stage)+'</span></div><div>'+niceDate(r.created)+'</div><div class="actions">'+(r.url?'<a class="btn primary" href="'+esc(r.url)+'" target="_blank">Open Job</a>':'')+(r.stage!=="Applied"?'<button class="btn" data-job-key="'+esc(r.key)+'">✓ Mark Applied</button>':'<span class="badge applied">✓ Applied</span>')+(r.resume_url?'<a class="btn" href="'+r.resume_url+'" target="_blank">View Resume</a>':'')+'<div class="menuWrap"><button class="btn" data-menu="1">•••</button><div class="menu"><button class="btn deleteBtn" data-delete-key="'+esc(r.key)+'">Delete</button></div></div></div></div>').join("")||'<div class="empty">No jobs in this view.</div>';let p=pipelineRows.filter(r=>!selectedDate||dateKey(r.created)===selectedDate);pipeCount.textContent=p.length+" runs";pipeTitle.textContent=selectedDate?"Pipeline Runs — "+labelDate(selectedDate):"Pipeline Runs — All Dates";pipes.innerHTML=p.map((r,i)=>'<div class="pipe"><b><span class="green">●</span> '+(i===0?"Latest Run":"Pipeline Run")+'</b><div>'+niceDate(r.created)+'</div><div>'+esc(r.discovered)+' jobs found · '+esc(r.eligible)+' eligible</div><div>'+esc(r.ready)+' ready to apply</div></div>').join("")||'<div class="empty">No pipeline records for this date.</div>'}
...
async function load(){let [a,p]=await Promise.all([fetch("/api/applications",{cache:"no-store"}).then(r=>r.json()),fetch("/api/pipelines",{cache:"no-store"}).then(r=>r.json())]);rows=a.applications;pipelineRows=p.pipelines||[];renderTabs();render()}
tabs.addEventListener("click",e=>{let b=e.target.closest("[data-date]");if(!b)return;selectedDate=b.dataset.date;renderTabs();render()});jobs.addEventListener("click",e=>{let m=e.target.closest("[data-menu]");if(m){m.nextElementSibling.classList.toggle("open");return}let b=e.target.closest("[data-job-key]");if(b){markApplied(b.dataset.jobKey,b);return}let d=e.target.closest("[data-delete-key]");if(d)deleteRow(d.dataset.deleteKey)});search.oninput=render;filter.onchange=render;todayLabel.textContent=new Date().toLocaleDateString([],{weekday:"short",month:"short",day:"numeric",year:"numeric"});load();setInterval(load,10000);
</script></body></html>""")
