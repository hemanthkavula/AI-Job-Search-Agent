...
</style></head><body><div class="app"><aside class="side"><div class="brand">💼 Auto Apply<small>Job Application Manager</small></div><div class="nav"><a class="active" href="#dashboard">⌂ &nbsp; Dashboard</a><a href="#jobsSection">▣ &nbsp; Jobs</a><a href="#pipelinesSection">⌘ &nbsp; Pipelines</a><a href="#schedulerCard">□ &nbsp; Scheduler</a><a href="https://github.com/hemanthkavula/AI-Job-Search-Agent" target="_blank">⚙ &nbsp; Settings</a></div><div class="sched" id="schedulerCard"><b>● Scheduler Active</b>Cloudflare schedule enabled</div></aside><main class="main" id="dashboard"><div class="top"><div><h1>👋 Good Morning!</h1><div class="muted">Here's your job application overview.</div></div><div style="display:flex;gap:10px"><div class="datepick" id="todayLabel"></div><a class="runBtn" href="https://github.com/hemanthkavula/AI-Job-Search-Agent/actions/workflows/daily-discovery.yml" target="_blank">▷ &nbsp; Run Pipeline Now</a></div></div>
...
<section class="section" id="jobsSection"><div class="sectionHead"><span id="jobsTitle">Jobs</span><span class="count" id="jobCount"></span></div><div class="row head"><div>Job Title</div><div>Company</div><div>Location</div><div>Source</div><div>Pipeline</div><div>Status</div><div>Date Created</div><div>Actions</div></div><div id="jobs"></div></section></main></div><script>
...
function render(){let q=search.value.toLowerCase(),f=filter.value;let x=rows.filter(r=>(!selectedDate||dateKey(r.created)===selectedDate)&&(!q||(r.company+" "+r.title+" "+r.portal).toLowerCase().includes(q))&&(!f||r.stage===f));counts(x);jobCount.textContent="Showing "+x.length+" jobs";jobsTitle.textContent=selectedDate?"Jobs — "+labelDate(selectedDate):"Jobs — All Dates";jobs.innerHTML=x.map(r=>'<div class="row"><div class="title">'+esc(r.title)+'</div><div class="company">'+esc(r.company)+'</div><div>'+esc(r.location||"—")+'</div><div>'+esc(r.portal)+'</div><div>'+esc(pipelineName(r.pipeline))+'</div><div><span class="badge '+cls(r.stage)+'">'+esc(r.stage)+'</span></div><div>'+niceDate(r.created)+'</div><div class="actions">'+(r.url?'<a class="btn primary" href="'+esc(r.url)+'" target="_blank">Open Job</a>':'')+(r.stage!=="Applied"?'<button class="btn" data-job-key="'+esc(r.key)+'">✓ Mark Applied</button>':'<span class="badge applied">✓ Applied</span>')+(r.resume_url?'<a class="btn" href="'+r.resume_url+'" target="_blank">View Resume</a>':'')+'<div class="menuWrap"><button class="btn" data-menu="1">•••</button><div class="menu"><button class="btn deleteBtn" data-delete-key="'+esc(r.key)+'">Delete</button></div></div></div></div>').join("")||'<div class="empty">No jobs in this view.</div>';let p=pipelineRows.filter(r=>!selectedDate||dateKey(r.created)===selectedDate);pipeCount.textContent=p.length+" runs";pipeTitle.textContent=selectedDate?"Pipeline Runs — "+labelDate(selectedDate):"Pipeline Runs — All Dates";pipes.innerHTML=p.map((r,i)=>'<div class="pipe"><b><span class="green">●</span> '+(i===0?"Latest Run":"Pipeline Run")+'</b><div>'+niceDate(r.created)+'</div><div>'+esc(r.discovered)+' jobs found · '+esc(r.eligible)+' eligible</div><div>'+esc(r.ready)+' ready to apply</div><div style="margin-top:12px"><button class="btn" data-pipeline="'+esc(r.cycle_id)+'">View Jobs →</button></div></div>').join("")||'<div class="empty">No pipeline records for this date.</div>'}
...
pipes.addEventListener("click",e=>{let b=e.target.closest("[data-pipeline]");if(!b)return;let id=b.dataset.pipeline;let match=rows.find(r=>r.pipeline===id);if(match){selectedDate=dateKey(match.created);renderTabs();render();document.getElementById("jobsSection").scrollIntoView({behavior:"smooth"})}});tabs.addEventListener("click",e=>{let b=e.target.closest("[data-date]");if(!b)return;selectedDate=b.dataset.date;renderTabs();render()});jobs.addEventListener("click",e=>{let m=e.target.closest("[data-menu]");if(m){m.nextElementSibling.classList.toggle("open");return}let b=e.target.closest("[data-job-key]");if(b){markApplied(b.dataset.jobKey,b);return}let d=e.target.closest("[data-delete-key]");if(d)deleteRow(d.dataset.deleteKey)});search.oninput=render;filter.onchange=render;todayLabel.textContent=new Date().toLocaleDateString([],{weekday:"short",month:"short",day:"numeric",year:"numeric"});load();setInterval(load,10000);
</script></body></html>""")
