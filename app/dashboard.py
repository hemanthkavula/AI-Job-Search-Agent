...
function renderTabs(){let dates=[...new Set(rows.map(r=>dateKey(r.created)).filter(Boolean))].sort().reverse();let today=dateKey(new Date());if(!selectedDate)selectedDate=today;let shown=[today,...dates.filter(d=>d!==today)].slice(0,6);tabs.innerHTML=shown.map((d,i)=>'<button class="tab '+(d===selectedDate?"active":"")+'" data-date="'+d+'">'+(d===today?"Today":labelDate(d))+'</button>').join("")+'<button class="tab '+(!selectedDate?"active":"")+'" data-date="">All Dates</button>'}
