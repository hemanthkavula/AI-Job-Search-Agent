...
pipes.addEventListener("click",e=>{let b=e.target.closest("[data-pipeline]");if(!b)return;selectedPipeline=b.dataset.pipeline;let run=pipelineRows.find(r=>r.cycle_id===selectedPipeline);selectedDate=run?dateKey(run.created):"";renderTabs();render();document.getElementById("jobsSection").scrollIntoView({behavior:"smooth"})});tabs.addEventListener("click",e=>{let allBtn=e.target.closest("[data-all-dates]");if(allBtn){selectedDate="";selectedPipeline="";renderTabs();render();return}let prev=e.target.closest("[data-prev]"),next=e.target.closest("[data-next]");if(prev||next){selectedPipeline="";let base=selectedDate||dateKey(new Date()),d=new Date(base+"T12:00:00");d.setDate(d.getDate()+(next?1:-1));selectedDate=d.getFullYear()+"-"+String(d.getMonth()+1).padStart(2,"0")+"-"+String(d.getDate()).padStart(2,"0");renderTabs();render();return}let b=e.target.closest("[data-date]");if(!b)return;selectedDate=b.getAttribute("data-date");selectedPipeline="";renderTabs();render()});jobs.addEventListener("click",e=>{let m=e.target.closest("[data-menu]");if(m){m.nextElementSibling.classList.toggle("open");return}let b=e.target.closest("[data-job-key]");if(b){markApplied(b.dataset.jobKey,b);return}let d=e.target.closest("[data-delete-key]");if(d)deleteRow(d.dataset.deleteKey)});datePicker.addEventListener("change",()=>{selectedDate=datePicker.value||"";selectedPipeline="";renderTabs();render()});search.oninput=render;filter.onchange=render;todayLabel.textContent=new Date().toLocaleDateString([],{weekday:"short",month:"short",day:"numeric",year:"numeric"});load();setInterval(load,10000);
</script></body></html>""")
